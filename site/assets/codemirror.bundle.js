...
// site/assets/completion-core.js
...
function scanSymbols(text, caret) {
  const items = [];
  const seen = /* @__PURE__ */ new Set();
  const add2 = (name2, kind, at) => {
    if (seen.has(name2)) return;
    if ((kind === "let" || kind === "const") && at > caret) return;
    seen.add(name2);
    items.push({ label: name2, kind, source: "doc", rank: 0 });
  };
  const fnRe = new RegExp("(?:^|\\n)\\s*(?:pub\\s+)?fn\\s+(" + IDENT + ")(\\." + IDENT + ")?", "g");
  for (const m of text.matchAll(fnRe)) {
    if (!m[2]) add2(m[1], "fn", m.index);
  }
  const tyRe = new RegExp("\\b(struct|enum|trait)\\s+(" + IDENT + ")", "g");
  for (const m of text.matchAll(tyRe)) add2(m[2], m[1], m.index);
  const bindRe = new RegExp("\\b(let|const|static)\\s+(?:mut\\s+)?(" + IDENT + ")", "g");
  for (const m of text.matchAll(bindRe)) {
    add2(m[2], m[1] === "let" ? "let" : "const", m.index);
  }
  return items;
}
var MAX_ITEMS = 40;
function filterItems(items, prefix) {
  ...
  const byRank = (a, b) => (a.rank || 0) - (b.rank || 0) || (a.label < b.label ? -1 : a.label > b.label ? 1 : 0);
  tier0.sort(byRank);
  tier1.sort(byRank);
  return tier0.concat(tier1).slice(0, MAX_ITEMS);
}

// site/assets/completion-data.js
var KEYWORDS = [
...
].map(([label, detail, doc2]) => ({ label, kind: "kw", detail, doc: doc2 }));
var PRIMITIVE_TYPES = [
  ["bool", "true / false"],
  ["i8", "-128 \u2026 127"],
  ["i16", "-32768 \u2026 32767"],
...
var BUILTINS2 = /* @__PURE__ */ new Set([
  "printf",
  "eprintf",
  "format",
  "panic",
...
var BiTag = Tag.define();
var rhoLanguage = StreamLanguage.define({
  name: "rho",
  startState() {
  ...
  token(stream, state) {
    ...
    if (stream.match(/[A-Za-z_][A-Za-z0-9_]*/)) {
      ...
      if (KEYWORDS2.has(w)) return "keyword";
      if (BUILTINS2.has(w)) return "builtin";
...
var rhoHighlight = HighlightStyle.define([
  { tag: tags.keyword, color: "#d886c8" },
  { tag: tags.typeName, color: "#7fb3d5" },
  { tag: tags.string, color: "#a3be8c" },
  { tag: tags.number, color: "#e8a04c" },
  { tag: tags.lineComment, color: "#8a8170", fontStyle: "italic" },
...
var MONO = '"SF Mono", ui-monospace, "Cascadia Code", Menlo, Consolas, monospace';
var rhoTheme = EditorView.theme({
  "&": {
