...
- `==`/`!=` on pointers compare identity (with `null`). On `string`,
  contents. On `bool`, ints, floats, chars — values. On `[]T` and `[N]T`,
  element-wise. On structs — element-wise. On enums — variant and payload.
...
- 0.0.5 is the freeze: the 0.1.0 self-hosted compiler must be writable in
  exactly the language 0.0.5 defines.
