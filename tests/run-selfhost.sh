...
run_one structpat 'enum Tok { End, Pair { k: i32, v: i32 }, } fn name(t: Tok) -> string { return match t { Tok.End => "end", Tok.Pair { k: k, v: v } => "kv", }; } fn sum(t: Tok) -> i64 { return match t { Tok.End => 0, Tok.Pair { k, v } => (k + v) as i64, }; } fn main() -> i32 { printf("{} {} {} {}\n", name(Tok.End), name(Tok.Pair(k: 2, v: 5)), sum(Tok.End), sum(Tok.Pair(k: 2, v: 5))); return 0; }' 'end kv 0 7'
run_one strstatic 'static mut A: string = "alpha"; static mut M: string = "m0"; fn quote(p: string, s: string) -> string { return p + s; } fn main() -> i32 { printf("{} {}\n", A, len(A) as i64); if A == "alpha" { printf("lit EQ\n"); } M = M + "-more"; printf("{} {} {}\n", M, len(M) as i64, quote("q:", A)); return 0; }' $'alpha 5\nlit EQ\nm0-more 7 q:alpha'
run_one generics 'fn id[T](x: T) -> T { return x; } fn wider[T](a: T, b: T) -> T { if a > b { return a; } return b; } fn sweep[T](tag: string, a: T, b: T) { printf("{} {} {}\n", tag, a + b, a - b); } fn main() -> i32 { let a: i8 = 100; let b: i8 = 90; let c: u8 = 200; printf("{} {} {}\n", id(42), wider(a, b), id(a) + 1); sweep("i8", a, b); sweep("u8", c, c); return 0; }' $'42 100 101\ni8 -66 10\nu8 144 0'
run_one overloads 'fn pick(a: i32) -> i32 { return a + 1; } fn pick(a: i64, b: i64) -> i64 { return a * 10 + b; } fn pick(s: string) -> string { return "[" + s + "]"; } fn assert_eq(a: string, b: string) { assert(a == b, "assert_eq failed"); return; } fn assert_eq(a: i32, b: i32) { assert(a == b, "assert_eq failed"); return; } struct Pt { x: i32, y: i32, } fn Pt.to_str(self: *Pt) -> string { return format("pt({}, {})", self.x, self.y); } enum Sh { Val(i32), } fn Sh.to_str(self: Sh) -> string { return match self { Sh.Val(v) => format("v{}", v), }; } fn main() -> i32 { printf("{} {} {}\n", pick(4), pick(4, 2), pick("hey")); assert_eq(pick("hey"), "[hey]"); assert_eq(pick(4), 5); let p = new Pt { x: 1, y: 2 }; printf("{} {}\n", p, format("|{}|", p)); let s: Sh = Sh.Val(9); printf("{}\n", s); return 0; }' $'5 42 [hey]\npt(1, 2) |pt(1, 2)|\nv9'
run_one variadic 'fn sum(ns: i64...) -> i64 { let mut t: i64 = 0; let mut i: usize = 0; while i < len(ns) { t += ns[i]; i += 1; } return t; } fn join(sep: string, parts: string...) -> string { let mut out: string = ""; let mut i: usize = 0; while i < len(parts) { if i > 0 { out = out + sep; } out = out + parts[i]; i += 1; } return out; } fn firsts(n: i64, ns: i64...) -> i64 { return n + sum(ns...); } fn main() -> i32 { let xs: []i64 = make([]i64, 3); xs[0] = 10; xs[1] = 20; xs[2] = 30; printf("{} {} {} {}\n", sum(), sum(1, 2, 3), sum(xs...), firsts(5, 1, 2)); printf("[{}]\n", join("-", "a", "b", "c")); let tags: []string = make([]string, 2); tags[0] = "x"; tags[1] = "y"; printf("{} {}\n", tags[1], len(tags) as i64); return 0; }' $'0 6 60 8\n[a-b-c]\ny 2'
run_one closures 'fn neg1(x: i32) -> i32 { return 0 - x; } fn apply(f: fn(i32) -> i32, x: i32) -> i32 { return f(x); } fn mk(k: i32) -> fn(i32) -> i32 { return fn(x: i32) -> i32 { return x + k; }; } struct Cell { v: i32, } fn main() -> i32 { let base: i32 = 10; let add: fn(i32) -> i32 = fn(x: i32) -> i32 { return x + base; }; printf("{} {} {}\n", add(5), apply(add, 1), apply(neg1, 3)); let add9: fn(i32) -> i32 = mk(9); printf("{}\n", add9(4)); let c: *Cell = new Cell { v: 1 }; let bump: fn(i32) -> i32 = fn(d: i32) -> i32 { c.v += d; return c.v; }; bump(4); printf("{}\n", c.v); return 0; }' $'15 11 -3\n13\n5'
